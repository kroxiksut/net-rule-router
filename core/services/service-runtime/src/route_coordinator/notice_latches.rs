...
//!
//! Same inherent impl, split across files; `pub(super)` methods here are
//! called from other modules under this one.
...
impl SecondaryRouteCoordinator {
    ...
    /// time would bury the log in one repeated answer. The cache has to be
    /// written either way — it is what lets routing survive the catch-all
    /// routes being stripped — so the latch is the write's own return value
