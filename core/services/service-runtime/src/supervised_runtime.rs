...
/// Budget for stopping the connection observers.
///
/// Its own step, ahead of the supervisor drain and outside that drain's shared
/// budget: an ETW `ControlTraceW(STOP)` joins a pump thread, and unsubscribing
/// from the filtering engine waits for an in-flight callback. Under the shared
/// budget this was the task that got detached, which is exactly the one that
/// must not be.
const CONN_OBSERVER_STOP_BUDGET: Duration = Duration::from_secs(10);
...
pub struct SupervisedRuntimeDeps {
    ...
    /// Stops the connection observers on teardown. Their own `Drop` is not
    /// enough: the observation task holds an `Arc` to the source, and a task
    /// the supervisor had to detach never releases it — so the ETW session and
    /// the machine-wide filtering-engine options would outlive the process.
    pub conn_observer_shutdown: Option<Arc<dyn Fn() + Send + Sync>>,
}
...
pub fn run_supervised_runtime(
    ...
    std::thread::scope(|scope| {
        ...
        // Stop the observers before the tasks are drained: an observation tick
        // that is mid-drain finishes against a source that is already
        // unsubscribed, which is harmless, while the reverse leaves a kernel
        // trace session and the machine-wide net-event options behind.
        if let Some(shutdown) = deps.conn_observer_shutdown.clone() {
            teardown_step("conn-observer-stop", CONN_OBSERVER_STOP_BUDGET, move || {
                shutdown()
            });
        }
