...
//!
//! The Win32 registry + process helpers here intentionally mirror the ones in
//! [`crate::app_path_resolver`] rather than share them: keeping this module
//! self-contained avoids destabilizing the load-bearing app-path resolver, and
//! the duplication is a few well-understood FFI calls.
...
/// The three `Uninstall` roots that hold installed-program metadata: the
/// native 64-bit view, the 32-bit `WOW6432Node` view (32-bit apps on 64-bit
/// Windows), and the per-user hive.
...
fn discover_from_installed_programs() -> Vec<VpnCandidate> {
    ...
    for (hive, root) in UNINSTALL_ROOTS {
        for sub in enum_subkeys(*hive, root) {
            ...
            if display_name.trim().is_empty() || !looks_like_vpn(&display_name) {
                continue;
            }
            // Best-effort exe path: DisplayIcon ("C:\...\app.exe,0") first,
            // then InstallLocation (a directory — kept as a hint even without a
            // specific exe, so the user can point the picker at it).
...
fn exe_from_display_icon(raw: &str) -> Option<String> {
    ...
    // `DisplayIcon` is often `"C:\path\app.exe,0"`; split the icon index off
    // the LAST comma only when what precedes it looks like a path+exe.
...
mod tests {
    ...
    fn exe_from_display_icon_strips_index_and_quotes() {
        assert_eq!(
            exe_from_display_icon(r#""C:\Apps\proton.exe,0""#).as_deref(),
            Some(r"C:\Apps\proton.exe")
        );
        assert_eq!(
            exe_from_display_icon(r"C:\Apps\nordvpn.exe").as_deref(),
            Some(r"C:\Apps\nordvpn.exe")
        );
        assert_eq!(
        ...
        // A bare .ico / non-exe icon yields no exe path.
