...
The background service builds the archive, in the data directory it owns and that ordinary users cannot open. You are then handed the file itself: the app copies it into a folder that is yours outright, `%TEMP%\NetRuleRouter`, and that copy is what the "Export diagnostic archive" button reports back and what "Open folder" opens. You never have to go into the service's directory, and you would not be able to.

Your copy also has two files the service-built one does not: `launcher-main.log` and `launcher-tray.log` — the app's own window-side logs, added because the service never reads anything from your user profile and so cannot see them. The `service-logs/` folder comes from the service itself and contains only what you are entitled to see.

You can ask for the same archive without the app: `nrr-cli diag export` requests it from the service and prints where it landed. It needs no administrator rights — the service builds the archive and hands it to whoever asked — but it does need the service to be running, and says so if it is not.
