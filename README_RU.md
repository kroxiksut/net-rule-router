...
> дополнительного (secondary) маршрута, — пресет не знает вашего личного
...
[`docs/ru/quickstart.md`](docs/ru/quickstart.md).
...
- **IPv6, CIDR-подсети, диапазоны IP, порты и протоколы пока не поддерживаются.**
  IPv6 обещан следующим выпуском — см. дорожную карту.
...
> поэтому сайт должен быть в правилах дополнительного (secondary) маршрута,
...
**Обещано, как только текущая версия стабилизируется: полноценный IPv6.**
Правило сможет называть IPv6-адрес или имя, которое в него разрешается, и
будет маршрутизироваться и защищаться так же, как сегодня работают правила по
IPv4. До тех пор правила действуют на IPv4, а IPv6 не даёт их обойти — правило,
которое вы написали, продолжает выполняться.

**Прорабатывается на будущее:** несколько сохранённых профилей и библиотеки
сценариев, 2+N адаптеров, более богатые типы правил (CIDR, порты, протоколы),
маршрутизация per-site / per-app по 3+ маршрутам и автоматическое
...
| [`docs/ru/packaging-windows.md`](docs/ru/packaging-windows.md) | Как собрать переносимый пакет для распространения |
