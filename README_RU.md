...
- **Четыре способа сопоставления трафика** — по домену (метка + все поддомены), по доменной зоне (TLD или внутренний суффикс, например `.ru` или `.intra`), по точному IPv4 или по приложению (имя процесса).
...
- **Точный IP** всегда побеждает зону, в которую входит: побеждает более узкое правило.
- Правило может сочетать совпадение по адресу **и** по приложению — должны выполняться оба условия (логическое **AND**).
- **CIDR-подсети и диапазоны IP появятся в следующей альфе.** Порты и
...
**Следующая альфа: CIDR-подсети и диапазоны IP.** Целая сеть, например
внутренние диапазоны корпоративного ВПН, направляется одним правилом, а не
правилом на каждый адрес. Слишком широкие диапазоны отклоняются, чтобы одно
правило не захватило заметную долю интернета.

**Прорабатывается на будущее:** несколько сохранённых профилей и библиотеки
сценариев, 2+N адаптеров, более богатые типы правил (порты, протоколы),
